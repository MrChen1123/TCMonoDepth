...
def cubes_2_maps(cubes):
    ...
    return cubes.view(b*t, c, h, w)


class MidasNet(torch.nn.Module):
    def __init__(self, args, features=256, non_negative=True):
        """Init.
        ...
        self.__mean = torch.tensor([0.485, 0.456, 0.406])
        self.__mean = self.__mean.view(1, 3, 1, 1)
        self.__std = torch.tensor([0.229, 0.224, 0.225])
        self.__std = self.__std.view(1, 3, 1, 1)

        self.pretrained, self.scratch = _make_encoder("resnext101_wsl", features, args)
    ...
    def forward(self, image):
        image = cubes_2_maps(image)
        image = image / 255.0
        self.__mean = self.__mean.to(image.device)
        self.__std = self.__std.to(image.device)
