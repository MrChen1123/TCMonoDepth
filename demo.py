...
def run(args):
    ...
    # state2 = torch.load(args.resume, map_location='cpu')
    ...
    for i in range(len(path_lists)):
        ...
        with torch.no_grad():
            for f in tqdm(path_lists[i]):
                ...
                # prediction.clip(min=0)
                output_list.append(prediction)

        # save mat
        for index, output in enumerate(output_list):
            path = os.path.join(args.output, scene_names[i], '%05d.mat' % index)
            save_mat(path, output)
